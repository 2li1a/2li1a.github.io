(function () {
  const SUPABASE_URL = "https://eaznorifsfzyldjigfdh.supabase.co";
  const SUPABASE_ANON_KEY = "sb_publishable_BeRYx2M9Iwg2aJxh90eyXg_vq3V-hit";

  const toText = (value, limit) => {
  ...
  const getVisitorKey = () => {
    ...
    if (window.crypto && typeof window.crypto.randomUUID === "function") {
      ...
      generated = `vk_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
    }
  ...
  const trackVisit = async () => {
    ...
    const endpoint = `${SUPABASE_URL.replace(/\/+$/, "")}/rest/v1/website_visits`;
    const payload = {
      page_path: toText(window.location.pathname || "/", 300) || "/",
      visitor_key: toText(getVisitorKey(), 200) || "unknown",
      referrer: toText(document.referrer || "", 1000),
      user_agent: toText(navigator.userAgent || "", 1000)
    };
    ...
    try {
      ...
      // Ignore tracking failures to avoid affecting page UX.
    }
  };
